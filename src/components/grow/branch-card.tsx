...
export function BranchCard({ branch }: BranchCardProps) {
  const imageUrl =
    branch.imageUrl ??
    (Array.isArray(branch.photoUrls) && branch.photoUrls.length > 0
      ? branch.photoUrls[0]
      : undefined);

  const whatsappNumber = branch.whatsapp ?? "6281234567890";
  const phoneDisplay = branch.phone ?? `+${whatsappNumber}`;

  let hoursDisplay = branch.hours ?? "Senin - Sabtu: 09:00 - 21:00";
  if (!branch.hours && branch.openingHours && typeof branch.openingHours === "object") {
  ...
  const facilitiesList = branch.facilities ?? [
    "Autoklaf Vakum Kelas B",
    "Rontgen Panoramik Digital",
    "Satu Rekam Medis Terhubung",
    "Parkir Luas & Aman",
  ];
      ...
      <div className="p-6 sm:p-7 flex-1 flex flex-col justify-between">
        <div className="space-y-5">
          ...
          <div className="flex items-start gap-2.5 text-xs sm:text-sm text-muted-foreground leading-relaxed">
            ...
            <span>{branch.address ?? "Jakarta Utara, DKI Jakarta"}</span>
          </div>
